...
private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {//сложение
	...
	for (int l = 0; l < 8; l++) {// сложение коэффициентов  первого и второго многочлена
		for (int i = 0; i < 8; i++) {
			if (ctep[l] == ctep1[i]) {
				koa[l] = koa1[i] + koa[l];
			}
...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {// вычитание
	...
	for (int l = 0; l < 8; l++) { //вычитание коэффициентов  первого и второго многочлена
		for (int i = 0; i < 8; i++) {
			if (ctep[l] == ctep1[i]) {
				koa[l] = koa[l] - koa1[i];
			}
...
private: System::Void button3_Click(System::Object^ sender, System::EventArgs^ e) {//умножение
	...
	Single::TryParse(textBox2->Text, System::Globalization::NumberStyles::Number, System::Globalization::NumberFormatInfo::CurrentInfo, o);// коэффициент
	Single::TryParse(textBox1->Text, System::Globalization::NumberStyles::Number, System::Globalization::NumberFormatInfo::CurrentInfo, o1);// степень
	int ctep[8], koa[8], f5;
	ctep[0] = a; ctep[1] = b; ctep[2] = c; ctep[3] = d; ctep[4] = r; ctep[5] = f; ctep[6] = g; ctep[7] = k;
	koa[0] = q, koa[1] = w, koa[2] = p, koa[3] = j, koa[4] = t, koa[5] = y, koa[6] = u, koa[7] = z;
	for (int i = 1; i < 8; i++) {// проверка, есть ли в многочлене одинаковые степени
	...
	for (int i = 0; i < 8; i++) {// умножение всех коэффициентов
		koa[i] = koa[i] * o;
	}
	textBox68->Text = String::Format("{0:F0}", koa[0]);//вывод коэффициентов
	textBox67->Text = String::Format("{0:F0}", koa[1]);
	textBox65->Text = String::Format("{0:F0}", koa[2]);
	textBox62->Text = String::Format("{0:F0}", koa[3]);
	textBox59->Text = String::Format("{0:F0}", koa[4]);
...
private: System::Void textBox2_TextChanged(System::Object^ sender, System::EventArgs^ e) {//коэффициент умножения 
}
private: System::Void textBox1_TextChanged(System::Object^ sender, System::EventArgs^ e) {//степень умножения 
}
private: System::Void button4_Click(System::Object^ sender, System::EventArgs^ e) {// деление
	...
	Single::TryParse(textBox11->Text, System::Globalization::NumberStyles::Number, System::Globalization::NumberFormatInfo::CurrentInfo, o);//коэффициент
	Single::TryParse(textBox7->Text, System::Globalization::NumberStyles::Number, System::Globalization::NumberFormatInfo::CurrentInfo, o1);// степень
	...
	textBox68->Text = String::Format("{0:F1}", koa[0] / o);//вывод коэффициентов
...
private: System::Void textBox11_TextChanged(System::Object^ sender, System::EventArgs^ e) {// коэффициент деления
}
private: System::Void textBox7_TextChanged(System::Object^ sender, System::EventArgs^ e) {// степень деления
}
};
}
